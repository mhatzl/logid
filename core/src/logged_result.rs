use crate::evident::event::intermediary::CapturedEvent;
use crate::{
    log_id::LogId,
...
pub struct ResultErr<E: From<LogId>> {
    pub error: E,
    pub event_entry_id: evident::uuid::Uuid,
}
...
impl<E: From<LogId> + Into<LogId>> ResultErr<E> {
    pub fn log_into<E2: From<LogId> + Into<LogId>>(
        ...
        let msg = format!(
            "'{}' caused by '{}'. Details see entry: '{}'.",
            &other_log_id, &self_log_id, causing_entry_id
        );
        ...
        .add_cause(CapturedEvent {
            event_id: self_log_id,
            entry_id: causing_entry_id,
        })
        .into()
    }
